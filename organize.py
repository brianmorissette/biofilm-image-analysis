...
def reorganize_raw_data_paired(source_dir, output_dir, dry_run=True):
    ...
    # Dictionary to store all found images: {(treatment, date, xy_num): {'biofilm': path, 'release': path}}
    image_registry = defaultdict(dict)
    
    print("=== Scanning files ===\n")
    
    # First pass: catalog all images
    ...
    # Second pass: identify complete pairs
    complete_pairs = []
    incomplete_images = []
    ...
    # Third pass: copy complete pairs
    if not dry_run:
    ...
    for key, images in complete_pairs:
        ...
        if dry_run:
            ...
            print(f"  Biofilm: {biofilm_file.relative_to(source_path)} -> biofilm/{biofilm_new_name}")
            print(f"  Release: {release_file.relative_to(source_path)} -> release/{release_new_name}")
            print()
        else:
...
if __name__ == "__main__":
    # Configure paths
    source_directory = "/Users/bmorissette/Documents/biofilm-image-analysis/raw_data"
    output_directory = "/Users/bmorissette/Documents/biofilm-image-analysis/raw_data_reorganized"
